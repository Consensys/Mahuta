...
import static net.consensys.mahuta.Settings.INDEXER_ELASTICSEARCH;
...
import net.consensys.mahuta.dao.index.ElasticSearchIndexDao;
...
@ConfigurationProperties(prefix = "mahuta.index")
...
public class IndexerConfiguration extends AbstractConfiguration {
     ...
     */
    @Bean
    @ConditionalOnProperty(value = "mahuta.index.type", havingValue = INDEXER_ELASTICSEARCH)
    public IndexDao elacticSearchIndexDao() throws ConnectionException {
        ...
        if (StringUtils.isEmpty(host))
            throw new IllegalArgumentException("mahuta.index.host" + ERROR_NOT_NULL_OR_EMPTY);
        if (StringUtils.isEmpty(port))
            throw new IllegalArgumentException("mahuta.index.host" + ERROR_NOT_NULL_OR_EMPTY);
        if (!this.getAdditionalParam(KEY_CLUSTER).isPresent())
            throw new IllegalArgumentException(
                    "mahuta.index.additional.clusterName" + ERROR_NOT_NULL_OR_EMPTY);
        ...
        try {
           ...
           log.info("Connected to ElasticSearch [host: {}, port: {}, cluster: {}] : {}", host,
                    port, additional.get(KEY_CLUSTER), transportClient.listedNodes().toString());
